          ...
          <div className="aspect-[16/10] relative overflow-hidden bg-slate-100">
            {item.thumbnail_url ? (
              ...
              <div className="w-full h-full flex items-center justify-center text-slate-300">
                {filterType === 'FOLLOWER' ? <Video className="w-12 h-12" /> : <Film className="w-12 h-12" />}
              </div>
            )}
            ...
            <div className="absolute bottom-4 left-4">
              <Badge className="bg-white/20 backdrop-blur-md text-white border-white/20 font-black text-[9px] px-3 py-1 rounded-lg">
                {filterType === 'FOLLOWER' ? 'تابعات' : 'REEL'}
              </Badge>
            </div>
          </div>
      ...
      {items.filter(i => i.type === filterType).length === 0 && (
        <div className="col-span-full py-32 text-center bg-slate-50/50 rounded-[3rem] border-2 border-dashed border-slate-200">
          <div className="bg-white w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-6 shadow-sm">
            {filterType === 'FOLLOWER' ? <Video className="h-10 w-10 text-slate-200" /> : <Film className="h-10 w-10 text-slate-200" />}
          </div>
    ...
    <div className="space-y-10 max-w-[1400px] mx-auto pb-10 px-4" dir="rtl">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        <div className="text-start">
          ...
          <h2 className="text-4xl font-black tracking-tight text-slate-900">إدارة التابعات</h2>
      ...
      <Tabs defaultValue="FOLLOWER" className="w-full">
        <div className="flex justify-center mb-10">
          <TabsList className="bg-slate-100 p-1.5 rounded-[2rem] h-16 w-full max-w-md border border-slate-200 shadow-sm">
            <TabsTrigger value="FOLLOWER" className="rounded-2xl font-black flex-1 gap-2 data-[state=active]:bg-white data-[state=active]:shadow-xl data-[state=active]:text-primary transition-all">
        ...
        {loading ? (
          ...
          <>
            <TabsContent value="FOLLOWER">{renderGrid('FOLLOWER')}</TabsContent>
          </>
        )}
      </Tabs>
          ...
          <div className="p-8 space-y-6 text-start overflow-y-auto flex-1 scrollbar-thin">
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-3">
                ...
                <Select value={type} onValueChange={(val) => setType(val || 'FOLLOWER')}>
                  <SelectTrigger className="h-14 rounded-2xl bg-slate-50 border-none font-bold px-6 text-slate-700">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="rounded-2xl border-none shadow-2xl">
                    <SelectItem value="FOLLOWER" className="font-bold rounded-xl py-3">تابعات</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-3">
