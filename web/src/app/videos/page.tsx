...
export default function VideosPage() {
  ...
  const [activeMediaFilter, setActiveMediaFilter] = useState<'ALL' | 'VIDEO' | 'REEL'>('ALL');
  ...
  // Compute dynamically filtered videos based on selected filter tab
  const filteredVideos = videos.filter((v: any) => {
    if (activeMediaFilter === 'ALL') return true;
    return v.type === activeMediaFilter;
  });
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-10 border-b border-slate-200/60 pb-16 text-start">
          ...
          <div className="flex items-center gap-6 bg-white/80 backdrop-blur-xl p-6 rounded-[2.5rem] border border-slate-200/60 shadow-xl shadow-slate-100/50">
            <div className="text-start">
              ...
              <span className="text-3xl font-black tracking-tighter text-slate-900">{videos.length} تقرير مرئي</span>
            </div>
            ...
            <div className="relative aspect-video max-h-[520px] w-full rounded-[3rem] md:rounded-[4rem] overflow-hidden group shadow-2xl border border-slate-200/50">
              ...
              <div className="absolute bottom-10 md:bottom-16 right-10 md:right-16 left-10 md:left-16 space-y-4 text-start z-10">
                <span className="inline-block bg-primary text-white font-black px-4 py-1.5 rounded-xl text-[9px] uppercase tracking-wider shadow-lg">
                  {featuredVideo.type === 'REEL' ? 'ريلز مميز' : 'عرض الآن'}
                </span>
        ...
        <section className="space-y-16">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-200/60 pb-8">
            <div className="flex items-center gap-4 text-start">
              ...
              <h3 className="text-3xl md:text-4xl font-black tracking-tight text-slate-900">أرشيف التغطيات</h3>
            </div>
            
            {/* Premium Light Glassmorphism Capsule Tabs */}
            <div className="flex items-center gap-2 bg-slate-200/60 p-1.5 rounded-2xl border border-slate-200/60 shadow-inner self-start md:self-auto backdrop-blur-md">
              <button 
                onClick={(e) => { e.stopPropagation(); setActiveMediaFilter('ALL'); }}
                className={`px-6 py-2.5 rounded-xl text-xs font-black transition-all duration-300 ${activeMediaFilter === 'ALL' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-500 hover:text-slate-800'}`}
              >
                الكل ({videos.length})
              </button>
              <button 
                onClick={(e) => { e.stopPropagation(); setActiveMediaFilter('VIDEO'); }}
                className={`px-6 py-2.5 rounded-xl text-xs font-black transition-all duration-300 ${activeMediaFilter === 'VIDEO' ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-slate-500 hover:text-slate-800'}`}
              >
                التقارير المرئية ({videos.filter(v => v.type === 'VIDEO').length})
              </button>
              <button 
