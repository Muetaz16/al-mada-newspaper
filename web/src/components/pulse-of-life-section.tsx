  ...
  return (
    <section className="space-y-12">
      <div className="flex items-center gap-6 border-b-2 border-slate-900 pb-6">
        ...
        <p className="text-slate-500 font-bold text-lg hidden md:block">سؤال وجواب عن جمال وصحة</p>
      </div>
      ...
      {isMarquee ? (
          ...
          <div className="flex gap-8 animate-marquee-slide">
            {marqueeItems.map((item, idx) => (
              <div 
                key={`${item.id}-${idx}`} 
                dir="rtl"
                className="w-[360px] shrink-0 bg-white p-8 rounded-[3.5rem] border border-slate-100/80 shadow-[0_20px_50px_-15px_rgba(0,0,0,0.03)] select-none pointer-events-none text-start space-y-6"
              >
                <div className="relative aspect-[4/3] rounded-[2.5rem] overflow-hidden shadow-2xl">
                  ...
                  <div className="absolute top-6 right-6">
                     <div className="bg-white/95 backdrop-blur-xl p-3.5 rounded-2xl shadow-xl text-primary border border-white/20">
                ...
                <div className="space-y-4">
                  ...
                  <h4 className="text-xl font-black text-slate-900 leading-snug tracking-tight line-clamp-3">
                    {item.question}
                  </h4>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          {items.map((item) => (
            <div key={item.id} className="group block space-y-6 bg-white p-10 rounded-[4rem] border border-slate-100 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] hover:shadow-2xl hover:shadow-primary/10 transition-all duration-500 ring-1 ring-slate-100 hover:ring-primary/20">
              <div className="relative aspect-square rounded-[3rem] overflow-hidden shadow-2xl pointer-events-none">
                ...
                <div className="absolute top-8 right-8">
                   <div className="bg-white/95 backdrop-blur-xl p-4 rounded-[1.5rem] shadow-2xl text-primary border border-white/20">
              ...
              <Link href={`/pulse-of-life/${item.id}`} className="block space-y-5 text-start cursor-pointer group/link">
                ...
                <h4 className="text-3xl font-black text-slate-950 leading-tight group-hover/link:text-primary transition-all duration-300 tracking-tighter">
                  {item.question}
                </h4>
              </Link>
            </div>
